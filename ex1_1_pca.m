...
figure(1);
hold on;
scatter(X(:,1),X(:,2));%plot the samples  
...
hold on;
p2=plot(X_rec(:, 1), X_rec(:, 2), 'ro');
for i = 1:size(X_norm, 1)
    drawLine(X_norm(i,:), X_rec(i,:), '--k', 'LineWidth', 1);
end
legend([p1 p2],'initial','recovered');
hold off
title('initial and recovered data');
fprintf('Program paused. Press enter to continue.\n');
pause;
